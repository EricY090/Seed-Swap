let submissionForm = document.getElementById('pepper-submission-form');
...
let sizeLowerBound = document.getElementById('sizeLowerBound');
let sizeUpperBound = document.getElementById('sizeUpperBound');
...
if(submissionForm){
    submissionForm.addEventListener('submit', function(event){
        ...
        const fields = [varietyName, originCountryCode, daysToHarvest, sizeLowerBound, sizeUpperBound, species, color, altNames];
        for(let field of fields){
            // check color field
            if(field.getAttribute('name') === 'varietyName'){
                ...
                if(isNaN(parseInt(field.value))){
                    addError("Invalid Heat Level", event);
                } else{
                    let heat = parseInt(field.value);
                    if(heat < 0 || heat > 6){
                        addError("Invalid Heat Level", event);
                    }
            ...
            else if(field.getAttribute('name') === 'sizeLowerBound'){
                if(isNaN(parseFloat(field.value))){
                    addError("Lower Bound Invalid", event);
                } else if(parseFloat(field.value) < 0){
                    addError("Upper Bound Invalid", event);
                }
            }
            else if(field.getAttribute('name') === 'sizeUpperBound'){
                if(isNaN(parseFloat(field.value))){
                    addError("Upper Bound Invalid", event);
                } else if(parseFloat(field.value) < 0){
                    addError("Upper Bound Invalid", event);
            ...
            else if(field.getAttribute('name') === 'altNames'){
                let altnames = field.value.split(',');
                for(let name of altnames){
                    if(name.trim().length == 0){
                        addError("Invalid Alt Name", event);
                    }
                }
            } 
            else if(field.getAttribute('name') === 'color'){
                if(!pepperColors.includes(field.value)){
                    addError("Invalid Color", event);
                }
            }
        }
        let lowerBoundNum = parseFloat(sizeLowerBound.value);
        let upperBoundNum = parseFloat(sizeUpperBound.value);
        if(!isNaN(lowerBoundNum) && !isNaN(upperBoundNum) && lowerBoundNum > upperBoundNum){
