 ...
 * @returns user object
 * @throws {string} invalid fields, xss vulnerability detected, user not found, pepper not found, pepper not Approved by moderator
 */
const addPepperToUserInv = async (userId, pepperName) => {
  if (!userId || !pepperName) throw "fields incomplete";
  if (typeof userId !== "string" || typeof pepperName !== "string")
  ...
  if (pepperName !== xss(pepperName))
    throw "pepperName is an xss vulnerability";
  ...
  if (!foundPepper) throw `Pepper ${pepperName} not found`;

  if (foundPepper.moderatorApproved !== true)
    throw `Pepper ${pepperName} not approved by moderator`;
...
const addPepperToUserWL = async (userId, pepperName) => {
  ...
  try {
    foundPepper = await peppersData.getPepperByNameAppr(pepperName);
  } catch (error) {
    throw error;
  }
